function [] = ulsq(n, snr, use_gurobi, save_dir, task_id, job_id)
    % Recover random signal from {-1, 0, 2, 3}^(signal_length) using different
    % recovery approaches. Experiments are conduncted with varying number
    % of measurements m between n and 2*n.
    %
    % Inputs:
    %   n: The length of the random signal
    %
    %   snr: Desired signal-to-noise ratio (in dB) of the additive Gaussian
    %   noise
    %
    ...
    save(strcat(save_dir, "/", string(job_id), "/", string(m)))
end
